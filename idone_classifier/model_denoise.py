...
def model_builder(
    ...
    if add_residual_between_models:
        previous_level = None
        current_level_result = None
        for i, x_level in reversed(list(enumerate(x_levels))):
            if previous_level is None:
                current_level_result = resnet_models[i](x_level)
                previous_level = current_level_result
            else:
                previous_level = \
                ...
                current_level_result = resnet_models[i](previous_level)
                previous_level = \
                    keras.layers.Add()(
                        [previous_level, current_level_result])
            x_levels[i] = current_level_result
    else:
        for i, x_level in enumerate(x_levels):
